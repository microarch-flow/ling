...
class TUI:
    ...
    def _build_keybindings(self) -> KeyBindings:
        ...
        def _(event):
            ...
            log.debug(f"[tui] enter pressed, text={repr(text)}")
            self._input_buffer.set_document(Document(""), bypass_readonly=True)
            event.app.create_background_task(self._on_user_input(text))
        ...
        @kb.add("c-d")
        def _(event):
            event.app.create_background_task(self._on_exit())
    ...
    def _get_status_text(self):
        ...
        return [("class:status", f" [ling] {self._cli_name}  {indicator}{suffix} ")]

    def append_output(self, text: str) -> None:
        """Append a line to the scrolling output area (safe to call from async tasks)."""
        def _do():
            ...
            self.app.invalidate()
        self.app.loop.call_soon_threadsafe(_do)
